...
namespace BaoLi.Application.Web.Areas.BaseManage.Controllers
{
    ...
    public class UserController : MvcControllerBase
    {
        ...
        /// <returns>返回分页列表Json</returns>
        [HttpGet]
        public ActionResult GetUserModelPageListJson(Pagination pagination, string queryJson)
        {
            var watch = CommonHelper.TimerStart();
            var data = userBLL.GetUserModelPageList(pagination, queryJson);
            var JsonData = new
            {
                rows = data,
                total = pagination.total,
        ...
        /// <returns>返回对象Json</returns>
        [HttpGet]
        public ActionResult GetCurrentJson()
        {
            var currentname = OperatorProvider.Provider.Current().UserId;
            var currentname123 = OperatorProvider.Provider.Current().UserName;
            return ToJsonResult(currentname123);
        }
        #endregion
