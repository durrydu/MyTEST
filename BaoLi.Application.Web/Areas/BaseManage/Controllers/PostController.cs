...
namespace BaoLi.Application.Web.Areas.BaseManage.Controllers
{
    ...
    public class PostController : MvcControllerBase
    {
        ...
        [AjaxOnly]
        [HandlerAuthorize(PermissionMode.Enforce)]
        public ActionResult SetAuthorizationMethod(string keyValue)
        {
            postBLL.SetAuthorizationMethod(keyValue);
            return Success("授权成功");
        }
        ...
        [ValidateAntiForgeryToken]
        [AjaxOnly]
        public ActionResult SaveForm(string keyValue, RoleEntity postEntity, List<FileModel> testFiles, List<FileModel> testFilesTwo)
        {
            postBLL.SaveForm(keyValue, postEntity, testFiles, testFilesTwo);
            return Success("操作成功。");
        }
        #endregion
    }
