...
namespace BaoLi.Application.Web.Areas.AuthorizeManage.Controllers
{
    ...
    public class PermissionRoleController : MvcControllerBase
    {
        ...
        public ActionResult GetPageList(Pagination pagination, string queryJson)
        {
            ...
            var data = permissionBLL.GetPageList(pagination, queryJson);
            var JsonData = new
            {
                rows = data,
        ...
        [AjaxOnly]
        public ActionResult SaveMember(string roleId, string userIds, string deleteUserIds)
        {
            permissionBLL.SaveMember(AuthorizeTypeEnum.Role, roleId, userIds, deleteUserIds);
            return Success("保存成功。");
        }
        /// <summary>
        /// 保存角色授权
