...
namespace BaoLi.Application.Web.Areas.AuthorizeManage.Controllers
{
    /// <summary>
    /// 创建人：姚栋
    /// 日 期：2018.05.30
    ...
    public class PermissionPostController : MvcControllerBase
    {
        ...
        {
            //var organizeEntity = organizeBLL.GetEntity("207fa1a9-160c-4943-a89b-8fa4db0547ce");
            //var data = departmentCache.GetList();
            var data = departmentBLL.GetListHasUser();
            var parentNode = data.FirstOrDefault(p => string.IsNullOrEmpty(p.ParentId));
            var treeList = new List<TreeEntity>();
            TreeEntity tree = new TreeEntity();
            tree.id = parentNode.DepartmentId;
            tree.text = parentNode.FullName;
            tree.value = parentNode.DepartmentId;
            tree.isexpand = true;
            tree.complete = true;
            ...
            foreach (DepartmentEntity item in data)
            {
                ...
                if (item.ParentId == parentNode.DepartmentId)
                {
                    tree.parentId = parentNode.DepartmentId;
                }
                else
                {
        ...
        [AjaxOnly]
        public ActionResult SaveMember(string postId, string userIds, string deleteUserIds)
        {
            permissionBLL.SaveMember(AuthorizeTypeEnum.Post, postId, userIds, deleteUserIds);
            return Success("保存成功。");
        }
        ...
        [ValidateAntiForgeryToken]
        [AjaxOnly]
        public ActionResult SaveAuthorizeAndPost(string postId, string projectColumnIds, string projectColumnNames)
        {
            try
            {
                permissionBLL.SaveAuthorizeAndPost(AuthorizeTypeEnum.Post, postId, projectColumnIds, projectColumnNames);
                return Success("授权成功。");
            }
            catch (Exception ex)
            {
        ...
        [ValidateAntiForgeryToken]
        [AjaxOnly]
        public ActionResult SaveAuthorizeAndPostAll(string keyValue)
        {
            permissionBLL.SaveAuthorizeAndPostAll(keyValue);
            return Success("授权成功。");
        }
        /// <summary>
        /// 保存岗位授权
