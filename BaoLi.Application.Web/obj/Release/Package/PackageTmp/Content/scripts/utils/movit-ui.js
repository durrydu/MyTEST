...
Loading = function (bool, text) {
    ...
    if (!!text) {
        ...
        top.$("#loading_manage").html("正在玩命为您加载…");
    }
...
tabiframeId = function () {
    var iframeId = top.$(".movit_iframe:visible").attr("id");
    return iframeId;
}
...
//日期:20180607
//描述:扩展下拉框赋值方法
$.fn.ComboBoxSetValueByIndex = function (index) {
    ...
    var $select = $(this);
    var $option = $("#" + $select.attr('id') + "-option");
    var value = $option.find("ul").eq(0).find("li").eq(index).attr("data-value");
    if ($.isNullOrEmpty(value)) {
        return;
    }
    return $select.ComboBoxSetValue(value);
...
$.fn.SetWebControls = function (data, isformatMoney) {
    if (typeof isformatMoney === 'undefined') {
        ...
        if (id.attr('id')) {
            ...
            switch (type) {
                ...
                case "text":
                    ...
                    var moneyFormat = id.attr('moneyFormat');
                    if (typeof moneyFormat !== 'undefined') {
                        var result = money_format(value, moneyFormat, 6, isformatMoney);
                        id.val(result)
...
$.fn.authorizeButton = function (moduleId) {
    ...
    if (pageId == undefined) {
        //如果从iframe里面拿不到就看看有没有写死的
        data = top.authorizeButtonData[pageId];
        if (data != undefined) {
            $.each(data, function (i) {
                $element.find("#" + data[i].EnCode).attr('authorize', 'yes');
            });
...
//获取枚举对应的描述
GetEnumDescription = function (cellvalue, options) {
    var result = "--";
    if (cellvalue != null) {
        result = options[cellvalue]
    }
    return result;
}
//使用原生弹出窗口，全屏
function sysf_openFullScreen(url) {
    if (url != null && url != "") {
        var newW = window.screen.availWidth;
        var newH = window.screen.availHeight - 60;
        return sys_openwin(url, "_blank", "status=no, menubar=no, toolbar=no, scrollbars=yes,location=no, left=0, top=0, width=" + newW + ", height=" + newH);
    }
    return false;
}
function sys_openwin(url, name, features, replace) {
...
//    /*
//    * 如何使用: var num=number_format(1234567.089, 2, ".", ",");//1,234,567.09
//    * 参数说明： 
//    * number：要格式化的数字
//    * decimals：保留几位小数
//    * dec_point：小数点符号
...
//直接舍去
function number_format(number, decimals, dec_point, thousands_sep) {
    /*如何使用:number_format(1234567.089, 2, ".", ",");//1,234,567.08
    * 参数说明：
    * number：要格式化的数字
    * decimals：保留几位小数
    * dec_point：小数点符号
...
function money_format(money, moneyFormat, decimals, isformat) {
    ...
    if (typeof moneyFormat !== 'undefined') {
        var result, _tempMoney;
        if (moneyFormat.indexOf("Y->M") > -1) {//元转成万元元再做格式转换
            _tempMoney = toFixed(floatObj.divide(money, 10000, decimals), decimals);


        } else if (moneyFormat.indexOf("M->Y") > -1) {//万元转成元再做格式转换
            _tempMoney = toFixed(floatObj.multiply(money, 10000, decimals), decimals);

        } else if (moneyFormat.indexOf("F") > -1) {//只做格式转换
            _tempMoney = money;
        }
        if (isformat) {
            result = number_format(_tempMoney, decimals, ".", ",")
        } else {
...
//金额格式化//
//让上传控件  变成只能下载  基本用于编辑页面 让他只读 姚栋 20180728
function fileControlReadOnly() {
    $(".filesbtns").remove();
    $(".webuploadDelbtn").remove();
