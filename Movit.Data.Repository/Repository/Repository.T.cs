...
namespace Movit.Data.Repository
{
    ...
    public class Repository<T> : IRepository<T> where T : class,new()
    {
        ...
        {
            string sidx = pagination.sidx ?? "";
            var sord = (pagination.sord ?? "asc").ToLower() == "asc" ? true : false;
            int total = pagination.records;
            var data = db.FindTable(strSql, sidx, sord, pagination.rows, pagination.page, out total);
            pagination.records = total;
            return data;
        }
        ...
        public T FindEntity(string strSql, DbParameter[] dbParameter)
        {
