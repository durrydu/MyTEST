...
namespace Movit.Application.Service.SystemManage
{
    ...
    public class CodeRuleService : RepositoryFactory, ICodeRuleService
    {
        ...
        {
            this.BaseRepository().Delete<CodeRuleEntity>(keyValue);
        }
        /// <summary>
        /// 保存规则表单（新增、修改）
        /// </summary>
        ...
        /// <param name="enCode">模板编码</param>
        /// <returns>单据号</returns>
        public string SetBillCodeByCode(string userId, string enCode, IRepository db = null)
        {
            //IRepository dbc = null;
            if (db == null)
            {
            ...
            UserEntity userEntity = db.FindEntity<UserEntity>(userId);
            CodeRuleEntity coderuleentity = db.FindEntity<CodeRuleEntity>(t => t.EnCode == enCode);
            //判断种子是否已经产生,如果没有产生种子先插入一条初始种子
            CodeRuleSeedEntity initSeed = db.FindEntity<CodeRuleSeedEntity>(t => t.RuleId == coderuleentity.RuleId);
            if (initSeed == null)
            {
            ...
            if (coderuleentity != null)
            {
                try
                {
                    ...
                    List<CodeRuleFormatEntity> codeRuleFormatList = coderuleentity.RuleFormatJson.ToList<CodeRuleFormatEntity>();
                    //看看是不是日期格式的编号规则
                    var numberFormat = codeRuleFormatList.FirstOrDefault(p => p.ItemType.ToString() == "1");
                    if (numberFormat != null)
                    {
                        //看看是不是精确到日的格式，如果是精确到日的格式 则按日来归0
                        //首先确定最大种子是否是隔天未归0的

                        switch (numberFormat.FormatStr)
                        {
                            case "mmdd":
                            case "yyyymmdd":
                            case "yymmdd":
                                ...
                                break;
                            case "yyyymm":
                                if ((maxSeed.ModifyDate).ToMonthDateString() != DateTime.Now.ToString("yyyy-MM"))
                                {
                                    isOutTime = true;
                                    maxSeed.SeedValue = 1;
                                    maxSeed.ModifyDate = DateTime.Now;
                                ...
                                break;
                            case "yyyy":
                                if ((maxSeed.ModifyDate).ToYearDateString() != DateTime.Now.ToString("yyyy"))
                                {
                                    isOutTime = true;
                                    maxSeed.SeedValue = 1;
                                    maxSeed.ModifyDate = DateTime.Now;
                    ...
                    coderuleentity.CurrentNumber = nextBillCode;
                    //this.BaseRepository().Update<CodeRuleEntity>(coderuleentity);
                    db.Update<CodeRuleEntity>(coderuleentity);
                }
                catch (Exception)
                {
