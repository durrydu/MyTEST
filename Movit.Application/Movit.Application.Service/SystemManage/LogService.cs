...
namespace Movit.Application.Service.SystemManage
{
    ...
    public class LogService : RepositoryFactory<LogEntity>, ILogService
    {
        ...
        public IEnumerable<LogEntity> GetPageList(Pagination pagination, string queryJson)
        {
            ...
            {
                string QueryJson = queryParam["QueryJson"].ToString();
                expression = expression.And(t => t.Module.Contains(QueryJson) || t.Description.Contains(QueryJson)
                    || t.Description.Contains(QueryJson)
                    || t.SourceObjectId.Contains(QueryJson)
                    || t.SourceContentJson.Contains(QueryJson)
                    || t.ExecuteResultJson.Contains(QueryJson));
            }
        ...
        public void WriteLogInterface(LogEntity logEntity)
        {
            try
            {
        ...
        {
            // Thread trWrite = new Thread(delegate()
            //{
            try
            {
                logEntity.LogId = Guid.NewGuid().ToString();
