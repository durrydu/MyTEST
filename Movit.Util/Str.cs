...
namespace Movit.Util
{
    ...
    public sealed partial class Str
    {
        ...
        #region 在固定长度插入固定字符串 姚栋
        public static string Insert(string text, int length, string insertText)
        {

            var insertCount = text.Length / length;
            var stratIndex = length;
            for (int i = 1; i <= insertCount; i++)
            {
                text = text.Insert(stratIndex, insertText);
                stratIndex = i * length + (i * insertText.Length);
            }
            return text;
        }
        #endregion
